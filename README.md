...
Папка `api/` — это серверная часть сайта: PHP-файлы принимают запросы от React-сайта и работают с MySQL-базой. На хостинге нужно скопировать `api/config.example.php` в `api/config.php` и заполнить данные MySQL: host, database name, user и password. Файл `api/config.php` добавлен в `.gitignore`, чтобы пароль от базы не попадал в GitHub.
...
3. После этого сайт будет брать каталог из MySQL, сохранять сообщения формы, корзину и заказы через PHP-файлы из папки `api/`.
...
Покупатели регистрируются через страницу личного кабинета или через форму в корзине. Без входа пользователь может просматривать каталог, но не может оформить заказ. Продавец `prodavec` видит отдельную версию кабинета со всеми заказами, а администратор `admin` видит упрощенную панель без витрины товаров — журнал действий пользователей и управление аккаунтами.
