  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const positionSmoothFactor = 0.5;
        const rotationBaseSmoothFactor = 0.38;
        const blendedFaceX = (noseBridge.x * 0.68) + (eyeCenterX * 0.2) + (templeMidX * 0.12);
        const templeYawAmount = rightTemple.z - leftTemple.z;
        const eyeYawAmount = rightEye.z - leftEye.z;
        const yawAmount = (templeYawAmount * 0.72) + (eyeYawAmount * 0.28);
        ...
        const yawVelocity = Math.abs(rawYaw - prevRawYawRef.current);
        prevRawYawRef.current = rawYaw;
        const yawFilterAlpha = THREE.MathUtils.clamp(0.26 + (yawVelocity * 2.8), 0.26, 0.62);
        yawFilteredRef.current += (yawAfterDeadzone - yawFilteredRef.current) * yawFilterAlpha;
        const targetYaw = yawFilteredRef.current;
        const templeAsymmetryX = (rightTemple.x - leftTemple.x) - (rightEye.x - leftEye.x);
        const anchorTargetX = ((blendedFaceX - 0.5) * 2 * halfWidthAtDepth) + (targetYaw * 0.11) + (templeAsymmetryX * 0.55);
        ...
        const rotationSmoothFactor = THREE.MathUtils.clamp(rotationBaseSmoothFactor + (Math.abs(targetYaw) * 0.34), 0.38, 0.72);
